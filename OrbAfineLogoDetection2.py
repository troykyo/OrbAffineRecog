...
def detect_logo(logo_path, photo_path):
    ...
    # Add affine transformation parameters to the feature_matches image (large yellow text)
    affine_text = f"Affine Matrix:\n{M}"
    x, y = 10, match_img.shape[0] - 100  # Bottom-left corner
    for i, line in enumerate(affine_text.split("\n")):
        cv2.putText(match_img, line, (x, y + i * 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 255), 4, cv2.LINE_AA)
