...
def detect_logo(logo_path, photo_path):
    ...
    # Add affine transformation parameters to the feature_matches image
    affine_text = f"Affine Matrix:\n{M}"
    x, y = 10, match_img.shape[0] - 50  # Bottom-left corner
    for i, line in enumerate(affine_text.split("\n")):
        cv2.putText(match_img, line, (x, y + i * 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
